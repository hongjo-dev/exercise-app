...
class Painter extends CustomPainter {
  final List posenetPoint; // 선을 그리기 위한 좌표값을 만듬.

  Painter({required this.posenetPoint});
        ...
        case 'leftShoulder':
          {
            for (int j = 0; j < posenetPoint.length; j++) {
              if (posenetPoint[j]['part'] == 'rightShoulder')
                canvas.drawLine(
                    Offset(posenetPoint[i]['left'] + ax, posenetPoint[i]['top'] + ay),
                    Offset(posenetPoint[j]['left'] + ax, posenetPoint[j]['top'] + ay),
                    paint); // 선을 그림.
